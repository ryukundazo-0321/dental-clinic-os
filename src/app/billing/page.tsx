              ...
              <div className="w-[420px] flex-shrink-0">
                <div className="bg-white rounded-xl border border-gray-200 shadow-lg sticky top-4 overflow-hidden">
                  <div className="bg-gray-900 text-white p-4">
                    <div className="flex items-center justify-between">
                      <div><p className="text-xs text-gray-400">患者名</p><p className="text-lg font-bold">{getName(selected)} 様</p></div>
                      <button onClick={() => setSelected(null)} className="text-gray-400 hover:text-white">✕</button>
                    </div>
                    <div className="flex items-end justify-between mt-3">
                      <div><p className="text-xs text-gray-400">合計点数</p><p className="text-3xl font-bold text-sky-400">{selected.total_points.toLocaleString()} <span className="text-sm">点</span></p></div>
                      <div className="text-right"><p className="text-xs text-gray-400">患者負担（{Math.round(selected.burden_ratio * 10)}割）</p><p className="text-2xl font-bold text-orange-400">¥{selected.patient_burden.toLocaleString()}</p></div>
                    </div>
                  </div>
                  {selected.ai_check_warnings?.length > 0 && (
                    <div className="bg-amber-50 border-b border-amber-200 px-4 py-2">
                      ...
                      {selected.ai_check_warnings.map((w, i) =>
                        w.includes("管理計画書") ? (
                          <div key={i} className={`flex items-center gap-2 py-1 ${selected.document_provided ? "opacity-50" : ""}`}>
                            <label className="flex items-center gap-1.5 cursor-pointer">
                              <input type="checkbox" checked={selected.document_provided || false} onChange={() => toggleDocumentProvided(selected)} className="rounded border-amber-400" />
                              <span className={`text-xs ${selected.document_provided ? "text-green-600 line-through" : "text-amber-600"}`}>{selected.document_provided ? "✅ 管理計画書を提供済み" : w}</span>
                            </label>
                            {!selected.document_provided && <Link href={`/management-plan?patient_id=${selected.patient_id}`} className="text-[10px] text-sky-600 underline hover:text-sky-800">📄 作成</Link>}
                          </div>
                  ...
                  <div className="border-t border-gray-200 p-4 bg-gray-50">
                    <div className="grid grid-cols-3 gap-2 mb-3 text-center">
                      ...
                      <div><p className="text-[10px] text-gray-400">{Math.round(selected.burden_ratio * 10)}割負担</p><p className="text-lg font-bold text-orange-600">¥{selected.patient_burden.toLocaleString()}</p></div>
                    ...
                    {selected.payment_status === "unpaid" ? (
                      <button onClick={() => markPaid(selected)} disabled={processing} className="w-full bg-green-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-green-700 disabled:opacity-50 shadow-lg shadow-green-200">
                        {processing ? "処理中..." : "💰 精算完了"}
                      </button>
                    ) : (
                      <>
                        <div className="text-center py-3 bg-green-100 rounded-xl"><p className="text-green-700 font-bold">✅ 精算済み</p></div>
                        <button onClick={() => printReceipt(selected)} className="w-full mt-2 bg-gray-800 text-white py-3 rounded-xl font-bold text-sm hover:bg-gray-700">🖨️ 領収書・明細書を印刷</button>
                      </>
                    )}
                  </div>
                </div>

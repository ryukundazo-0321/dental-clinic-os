...
//
// 設計方針：
// - fee_codeはmatched.patientsの実データから取る（ハルシネーション排除）
// - OpenAIはパターン命名・算定漏れ指摘・insightsだけ担当
// - フロントはanalysis（OpenAI）+ matched_patients（実データ）を受け取る
// ============================================================
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    // fee_codeはOpenAIに返させない（ハルシネーション排除）
    // OpenAIはパターン命名・算定漏れ指摘・insightsだけ担当
...
`.trim();
    ...
    // === 7. レスポンス ===
    // analysis  : OpenAIによるパターン命名・算定漏れ・インサイト
    // matched_patients: 実UKEデータ（fee_codeはここから取る）
    return NextResponse.json({
      success: true,
      analysis,

...
export async function POST(req: NextRequest) {
  try {
    ...
    const { image_base64, image_url, media_type } = body;
...
【画像形式への対応】
- デジタルX線（パノラマ・デンタル）: 最も詳細な分析が可能
- モニター撮影写真（タブレット等でモニターを撮影）: グレア・反射・斜め角度があっても読み取る
- 口腔内カラー写真: 色調・形態から視覚的所見を報告
→ いずれの形式でも見える情報を最大限活用して所見を報告すること
...
4. 親知らず(18,28,38,48)の状態も必ず確認
5. 特に重要な所見をnotable_findingsに列挙
        ...
        messages: [
          { role: "system", content: systemPrompt },
          {
            ...
            content: [
              {
                type: "text",
                text: `この歯科画像を専門医レベルで分析してください。

画像形式（X線・モニター撮影・口腔内写真など）に関わらず、見える情報を全て活用して所見を報告してください。

分析手順：
1. 上顎右(18→11)→上顎左(21→28)→下顎左(31→38)→下顎右(41→48)の順に全歯をスキャン
2. 補綴物・欠損・う蝕・根管治療・歯周病変を全て記録
3. 特に重要な所見（即時対応必要）をnotable_findingsに記載

  ...
  try {
    ...
    try {
      ...
      whisperFormData.append("prompt",
        ...
        "「痛みはどうですか？」「冷たいものがしみます。」" +
        "う蝕 C1 C2 C3 C4 FMC CAD/CAM冠 CR充填 インレー 抜髄 根管治療 " +
        "感根治 根充 TEK SC SRP PMTC TBI P検 BOP 印象 咬合採得 形成 装着 " +
        "ロキソニン フロモックス カロナール メイアクト サワシリン " +
        "右上 左上 右下 左下 1番 2番 3番 4番 5番 6番 7番 8番 " +
        "歯周炎 歯髄炎 根尖性歯周炎 智歯周囲炎 浸麻 伝麻"
      );
...
主な補正例:
- 歯番号: 6版/六版→6番、五番→5番、市場→4番、碁盤→5番、1万→1番、に版→2番
- 処置: FMS→FMC、CR充電→CR充填、印傷→印象、テク→TEK、形勢→形成
        バツ随/罰随→抜髄、感根地/完根地→感根治、コン中/根中→根充
        エス下→SC、SOP→SRP、PMDC→PMTC、TDI→TBI
- 用語: 浸魔/新魔/親魔→浸麻、伝魔→伝麻、ラバータム→ラバーダム
        歯肉縁上/下→歯肉縁上/縁下、P券→P検、ポケット地→ポケット値
- 薬名: 録そにん/六曽人→ロキソニン、フロモクス→フロモックス
        カロナール/加論ある→カロナール、砂割新→サワシリン
        メイアクト、ジスロマック、ボルタレン
- 材料: パラジウム→金パラ、CADカム→CAD/CAM、ジルコ→ジルコニア
...
#### S（主訴）のルール ★重要★
- 患者の主訴は①②③の番号付きで**全て**列挙する
- 1つの主訴でも①を付ける
- 【既存の主訴】が提供されている場合は、既存の番号と内容を**必ず保持**したまま、新しい主訴を追番で追加する
- 既存の主訴を削除・統合・書き換えてはならない
- 音声から主訴を特定できなかった場合は "undetected": true を出力に含める
- 例：
  既存: ①右下奥歯が冷たいものでしみる
  新しい録音で「左上の詰め物が取れた」が判明 → 出力:
  ①右下奥歯が冷たいものでしみる
  ②左上の詰め物が取れた

#### O（所見）
- 歯番号＋所見＋実施した全処置を具体的に。略語OK

#### A（診断）
- 歯番号＋確定診断名を全て列挙

#### P（計画）のルール ★重要★
- **「本日実施した処置」と「次回予定」を明確に分けて記載する**
- 必ず以下の形式で出力する:
  【本日】（今日実際に行った処置＋処方薬）
  【次回】（次回以降に予定している処置＋予約）
- 「次回〜する」「来週〜予定」等の表現は全て【次回】に入れる
- 今日行っていない処置を【本日】に含めてはならない
- 例:
  【本日】CR充填 #46, SC全顎, Rp) ロキソニン60mg 3T 3日分
...
- caries: 未治療う蝕の発見
- missing: 欠損歯（抜歯済み、先天性欠如）
- br_abutment: ブリッジの支台歯（被せが架かる両端の歯）
- br_pontic: ブリッジのポンティック（欠損部のダミー歯）

#### ブリッジの記録ルール ★重要★
- 「ブリッジ」「Br」と言及された場合、支台歯とポンティックを正しく区別する
- 欠損歯（歯がない部分）= br_pontic、両隣の支える歯 = br_abutment
- 例: 「右下5-7ブリッジ」→ #45: br_abutment, #46: br_pontic, #47: br_abutment
- 例: 「左上4-6Br」→ #24: br_abutment, #25: br_pontic, #26: br_abutment
- 3歯以上のブリッジでは中間の歯が全て br_pontic になる
- 例: 「右下4-7Br」→ #44: br_abutment, #45: br_pontic, #46: br_pontic, #47: br_abutment
...
${existingSoapS ? `【既存の主訴（必ず保持すること）】\n${existingSoapS}\n\n` : ""}【音声書き起こし】
${safeTranscript}

以下のJSON形式で出力:
{
  "corrected_transcript": "補正後の会話テキスト（誤認識を修正したもの）",
  "soap_s": "患者の主訴・症状（①②③の番号付きで全て列挙。既存の主訴がある場合は必ず保持し追番で新しい主訴を追加）",
  "soap_s_undetected": false,
  "soap_o": "所見・実施した処置の詳細（歯番号含む）",
  "soap_a": "歯番号+診断名（全て列挙）",
  "soap_p": "【本日】今日実施した処置+処方薬\n【次回】次回予定の処置+予約",
  "tooth_updates": {"歯番号": "ステータス"},
  "procedures": ["処置名"],
  "diagnoses": [{"name": "傷病名", "tooth": "歯番号", "code": "コード"}]
}

soap_s_undetected: 音声から主訴を明確に特定できなかった場合はtrue。特定できた場合はfalse。
soap_p: 必ず「【本日】」と「【次回】」で分けて記載。次回の処置を【本日】に含めない。
...
コード: K021=C1, K022=C2, K023=C3, K024=C4, K040=Pul, K045=Per, K050=G, K051=P, K081=Perico, K120=Hys, K003=破折`;
  ...
  return NextResponse.json({
    ...
    soap_s_undetected: soapData.soap_s_undetected === true,

...
- ★重要: 音声書き起こしテキストに含まれる歯番号をそのまま使うこと。推測で変更しない。
  例: 書き起こしに「5番」とあれば45（右下の場合）。「奥歯だから6番だろう」と勝手に変えない。
  例: 書き起こしに「6番」とあれば46（右下の場合）。Whisperが認識した番号を信頼する。
...
「右下の奥歯」→ 文脈から46,47,48のいずれか判断（番号が明示されていない場合のみ推測可）
「前歯」→ 文脈から11-13,21-23,31-33,41-43のいずれか判断（番号が明示されていない場合のみ推測可）
★番号が明示されている場合は必ずその番号を使う。変更禁止。
...
正確な処置名を使用してください:
FMC, CAD/CAM冠, 前装冠, インレー, CR充填, ブリッジ, 抜髄, 感根治, 根充, SC, SRP, 普通抜歯, 難抜歯, 装着, TEK, 印象, 連合印象, 浸麻, 咬合採得, 処方`;

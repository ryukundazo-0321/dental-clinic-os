...
## A欄の記述ルール（★保険請求に直結するため正確に）
- 歯番号＋確定診断名を**全て**列挙（漏れ厳禁）
- 処置に対応する傷病名:
  CR充填/インレー → う蝕(C1〜C4) 明示あればその段階
  抜髄 → 急性歯髄炎(Pul)または慢性歯髄炎
  感根治 → 根尖性歯周炎(Per)
  SC/SRP → 慢性歯周炎(P)
  抜歯(智歯) → 智歯周囲炎(Perico)
  知覚過敏処置 → 象牙質知覚過敏症(Hys)
- 複数歯は各歯ごとに記載: "#46 C2(う蝕)、#36 Per(根尖性歯周炎)"
...
## 重要
- 音声の内容に**ない処置や診断を追加しない**
- 「次回〜」と言われたら次回予定に反映
- 処方薬の言及があれば漏れなく記載

出力はJSON形式で:
{
  "soap_a": "A欄（歯番号+診断名を全て列挙）",
...
患者: ${ctx.patient ? JSON.stringify(ctx.patient) : "(なし)"}`;
    ...
    // ★ gpt-4oを使用（miniではなく）。治療計画はminiでもOK
    const model = step === "treatment_plan" ? "gpt-4o-mini" : "gpt-4o";

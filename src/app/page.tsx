...
type ToastItem = { id: string; title: string; body: string | null; type: string };
...
export default function Home() {
  ...
  const [notifs, setNotifs] = useState<ClinicNotif[]>([]);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("dashboard-realtime")
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "notifications" }, (payload) => {
        const n = payload.new as ClinicNotif;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 pointer-events-none">
        {toasts.map(t => (
          <div key={t.id} className="bg-white border border-gray-200 rounded-2xl shadow-2xl px-4 py-3 flex items-start gap-3 w-72 pointer-events-auto">
            <span className="text-xl shrink-0">{notifIcon(t.type)}</span>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-gray-900">{t.title}</p>
              {t.body && <p className="text-xs text-gray-500 mt-0.5">{t.body}</p>}
            </div>
          </div>
        ))}
      </div>
